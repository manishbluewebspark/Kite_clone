...
interface OrderModalProps {
  /** Symbol shown in the header, e.g. "HDFCBANK" */
  symbol?: string;
  /** BSE / NSE quote values shown under the symbol */
  bsePrice?: number;
  nsePrice?: number;
  /** Starting price used to prefill the Price field */
  startPrice?: number;
  /**
   * true  -> opens orange (used for Sell / "S" button)
   * false -> opens blue   (used for Buy / "B" button)
   * The header toggle still lets the user flip this manually afterwards.
   */
  defaultOrange?: boolean;
  /** Called when the user hits Cancel or the backdrop / X is clicked */
  onClose?: () => void;
}
...
export default function OrderModal({
  ...
  // Keep the accent synced with whichever button (Buy/Sell) opened this
  // instance, unless the user has already flipped it manually via the toggle.
  useEffect(() => {
  ...
  const [intraday, setIntraday] = useState(true); // Intraday vs Longterm for Regular/Iceberg
  const [orderType, setOrderType] = useState<"market" | "limit">("limit");
  const [legs, setLegs] = useState(2);
  const [triggerPrice, setTriggerPrice] = useState(791.05);

  const accent = isOrange ? "#FF7A1A" : "#2563EB"; // orange-500ish vs blue-600
  ...
  return createPortal(
    <>
      ...
      {/* Modal positioned at bottom center */}
      <div
      ...
      >
        ...
        >
          ...
          <div className="px-4 pb-4 flex flex-col gap-2">
            <button
              className="w-full py-2 rounded-sm text-white text-[14px] font-medium transition-colors duration-300"
              style={{ backgroundColor: accent }}
            >
              Buy
            </button>
