...
// // GET /api/instruments/search?q=NIFTY&exchange=NFO&limit=30
// router.get("/search", authMiddleware, (req, res) => {
//   try {
//     const { q, exchange, limit } = req.query;

//     if (getKiteInstrumentCount() === 0) {
//       return res.status(503).json({
//         success: false,
//         message: "Instrument list loading, try again in a moment",
//       });
...
//     const results = searchKiteInstruments(q, {
//       exchange,
//       limit: limit ? parseInt(limit) : 30,
//     });
...
//     const allResults = searchKiteInstruments(q, { exchange });
//     const pageNum = Math.max(1, parseInt(page));
//     const limitNum = Math.max(1, parseInt(limit));
//     const start = (pageNum - 1) * limitNum;
...
// // POST /api/instruments/bulk-ltp
// // Body: { instruments: [{ symbol, exchange, token }] }
// router.post("/bulk-ltp", authMiddleware, async (req, res) => {
//   try {
//     const { instruments } = req.body;
...
//     const quotes = await getKiteBulkLTP(instruments);
//     res.json({ success: true, data: quotes });
//   } catch (err) {
//     console.error("Bulk LTP error:", err.message);
//     res.status(500).json({ success: false, message: err.message });
//   }
// });
...
// router.post("/refresh", authMiddleware, async (req, res) => {
//   try {
//     await loadKiteInstruments(true);
//     res.json({ success: true, count: getKiteInstrumentCount() });
//   } catch (err) {
//     res.status(500).json({ success: false, message: err.message });
//   }
// });
